...
每个 grow 单元必须产出并维护质量门禁和目标覆盖表，不能漏题。
每次备份、重跑、清理或替换工作目录之前，必须基于 file-native usage 和运行报告分析缓存命中率；低命中率要进入系统层问题判断，而不是被备份掩盖。
```
...
质量门禁：由 grow 产出的通过条件、证据要求和目标覆盖状态。
...
9. readiness 满足时，feng 提出 hatch 建议并等待必要确认。
10. hatch 提取稳定能力包。
11. 能力包运行并产生反馈单元。
12. 反馈路由判断本地吸收、上游提议、忽略或待确认。
13. 被采纳的反馈进入下一轮 grow。
```

这个闭环可以内部复杂，但产品第一心智必须简单：用户提出一个智能行为，持续给材料和反馈，直到 feng 说“成了”。

Hatch 是 readiness 转换，不是绕过 grow 的并列动作。用户可以请求检查是否可 hatch，但不能用 hatch 命令跳过目标覆盖、质量门禁、运行契约和验证证据。readiness 满足后，系统也不应该无限 grow，而应该收敛到 hatch 建议、边界确认和能力包提取。
...
压缩只能改变活跃表示，不能改写完整事实。
稳定前缀应该保持 cache-aware：system prompt、核心规则、工具说明、skill 说明和长期不变的契约要尽量逐字节稳定，动态事实、时间戳、随机 id、路径差异和本轮反馈应放在后续可变段。
```
...
每次编译都应该产出可检查文件，包括 message list 本身、来源说明和缓存观测依据。feng 需要把缓存命中率视为长程任务健康指标：命中率长期偏低，通常说明 message list 每轮被重新洗牌、稳定规则被动态内容打断、工具面变化没有被明确分段，或者 provider usage 没有被完整记录。
...
### Quality Gate Synthesizer

Quality Gate Synthesizer 负责在 grow 中生成和维护质量门禁。它不是一套固定评分模板，而是把用户目标、材料、目标世界接口、运行契约、反馈和 DoD 转成当前 grow 单元自己的门禁。
...
识别材料和目标世界中的隐含要求。
维护目标覆盖表。
把要求转成硬门槛、软指标、人工确认项或待澄清问题。
为每个门禁项定义所需证据。
记录门禁项来源、状态、变更和豁免原因。
在门禁未覆盖关键目标时阻止 ready_to_hatch。
把门禁失败转入 grow 议程或反馈路由。
```

这就是系统层面的“不能漏题”。feng 不能只完成自己想到的部分，也不能把小说、boss、小车等不同目标世界套进一张通用评价表。默认门禁 skill 可以提供维度，但具体门禁必须由当前 grow 单元产出。
...
如果一个用户目标、关键材料约束或重要反馈没有进入任何处理状态，readiness 不能通过。
...
小说场景至少存在三层门禁，但每层门禁都应由对应 grow 过程产出，而不是写死在 feng 里。

```text
feng 系统层门禁
  判断 grow kernel、message list、hatch、feedback router、隐私和版本边界是否可信。

xiaoshuo 领域 agent 门禁
  判断小说 agent 是否能接收作品材料、产出章节/修订、维护长篇上下文、处理作者反馈、归因失败，并迁移到不同作品项目。

libai-chongshengle 作品层门禁
  判断当前作品是否覆盖题材承诺、人物设定、世界规则、章节目标、伏笔继承、风格边界和作者反馈。
```

这些只是门禁维度示例。真正的门禁项必须绑定当前目标和材料。比如 `libai-chongshengle` 的门禁应该由 xiaoshuo 根据“李白重生了”的作品设定、作者目标、已写章节和反馈生成；`xiaoshuo` 的门禁应该由 feng 根据“小说 agent”的领域目标、样例作品和运行证据生成。

作品层门禁可以支持 agent 判断“结构性质量是否通过”或“是否可作为候选发布稿”，但不应该让 agent 单独宣称“这就是好小说”。小说的最终合格性在早期仍需要作者或读者验收。
...
质量门禁和目标覆盖表如何文件化，才能防止漏题但不变成沉重配置？
```
